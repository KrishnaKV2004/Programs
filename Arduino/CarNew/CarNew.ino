...
void setup() {
  ...
  // Release motors initially (set them to coast mode)
  motor1.run(RELEASE);
  motor2.run(RELEASE);
  motor3.run(RELEASE);
  motor4.run(RELEASE);
...
void forward() {
  steeringAngle = 90; // Adjust the angle for left turn (example: 0 degrees)
...
void right() {
  steeringAngle = 0; // Adjust the angle for right turn (example: 90 degrees)
...
void left() {
  steeringAngle = 180; // Adjust the angle for left turn (example: 0 degrees)
